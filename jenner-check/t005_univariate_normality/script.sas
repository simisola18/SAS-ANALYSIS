/* Adapted from P2686827-CODE.sas (lines 288-297): the normality test for
   region's effect on GLH-per-learner via PROC UNIVARIATE (qqplot, ppplot,
   histogram, inset), unmodified in its options. Total_data_all is rebuilt
   inline from the same 12-row real FE-college sample used in t004 (4 rows
   each from East Midlands, East of England and Greater London, from
   IMAT5168-FE.csv). The VAR list is narrowed to one analysis variable
   (GLHHours_per_Learner_year1, the original's first) instead of all seven,
   purely to keep the bundle's ODS-plot volume small for a self-check run;
   the PROC UNIVARIATE statement, options, and plot requests are otherwise
...
proc univariate
	...
	class region;
	var GLHHours_per_Learner_year1;
